...
public class NumbersFinder {
	...
	// find "value" in the sorted array of "items" using Binary search in O(log
	// n)
	private Item searchSortedItems(ArrayList<Item> items, Integer value,
			int lower, int upper) {
		if (lower > upper)
			return null;
	...
	// Simply calculates difference between the target value and each item in
	// the array; O(n)
	...
	private ArrayList<Item> quicksort(ArrayList<Item> items) {
		...
		ArrayList<Item> lesser = new ArrayList<>();
	...
	public static void main(String[] args) {
		...
		if (numbers != null) {
			System.out.println("First Number Found = "
					+ numbers.get(0).getValue() + " ; Index = "
					+ +numbers.get(0).getIndex());
			System.out.println("Second Number Found = "
					+ numbers.get(1).getValue() + " ; Index = "
					+ +numbers.get(1).getIndex());
		} else {
